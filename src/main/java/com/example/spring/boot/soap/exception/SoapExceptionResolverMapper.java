...
public class SoapExceptionResolverMapper extends AbstractEndpointExceptionResolver {
    ...
    /**
     * Searches for a the exception class or the closest superclass of the
     * exception.
     * 
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    protected EndpointExceptionResolver findResolver(Exception ex) {
        Optional min = resolvers.keySet().stream().map(i -> new ImmutablePair<>(i, getDepth(i, ex)))
                .filter(p -> p.getRight() >= 0).min((o1, o2) -> signum(o1.getRight() - o2.getRight()));
    ...
    /**
     * Return the depth to the superclass matching. {@code 0} means ex matches
     * exactly. Returns {@code -1} if there's no match. Otherwise, returns
     * depth. Lowest depth wins.
     * <p>
     * <p>
